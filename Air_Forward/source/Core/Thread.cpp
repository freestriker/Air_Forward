...
void Core::Thread::LogicThread::OnRun()
{
	...
	{	
		...
		IterateByStaticBFS(Component::Component::ComponentType::BEHAVIOUR);
		auto targets = IterateByStaticBFSWithRecord(Component::Component::ComponentType::BEHAVIOUR);
		Utils::Log::Message("Get " + std::to_string(targets.size()) + " targets.");
